    ...
    <div class="px-10 py-20">
        ...
        <div class="flex flex-col">
          <label class="block">
            ...
            <span v-if="v$.date.$error" class="text-red-500">
              <span v-if="v$.date.required.$invalid">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
        ...
        <div class="flex justify-between mt-10 mr-20">
          <confirmation @ChildModalsubmit="submitEventUpdate" type="submit" class="bg-green-700 disabled:opacity-50 text-white rounded"
        ...
        <!--Delete Event button-->
        <div class="flex justify-between mt-10 mr-20">
          <confirmation @ChildModalsubmit="submitDeleteEvent" type="submit" class="bg-red-700  disabled:opacity-50 text-white rounded"
            :disabled="user.role === 'viewer'">
            Delete Event
          </confirmation>
          <!--Go back button-->
...
<!-- Begin transition from options to composition API using script setup-->
<script setup>
// Importing required functions and modules such as ref, required
...
// The webpage wull be able to use notifications
...
// Defining the refs and reactive data using Composition API
const event = ref({
  name: null,
  description: null,
  date: null,
...
onMounted(async () => {
  try {
    // Make API calls to load event data, attendees, and services
...
// This function will work to update the function.
async function submitEventUpdate() {
  console.log('submit event update')
  ...
  if (v$.value.$error) {
    ...
    return; // Code will stop if the validation will fail
  }
  else{
  ...
  try {
    ...
    router.push('/findevents'); // Redirect page after sucessful submission
...
// Function to delete the event If there are no attendees 
async function submitDeleteEvent () {
  console.log('submit event delete')
  try {
    if (event.value.attendees.length != 0) {
      toast.info('Event cannot be deleted because it has attendees.');
      return;
    }
