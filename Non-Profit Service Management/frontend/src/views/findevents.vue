...
// Method to handle search submissions
const handleSubmitForm = async () => {
  ...
  if (searchBy.value === 'Event Name') {
    ...
    if (eventDate.value) {
      try {
        const eventDateFormatted = new Date(eventDate.value).toISOString().substring(0, 10);
        const query = { searchBy: 'date', eventDate: eventDateFormatted };
        const response = await searchEvents(query);
        events.value = response;
      } catch (error) {
        toast.error('Error searching events', error);
